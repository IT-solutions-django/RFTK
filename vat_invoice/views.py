...
from invoice.models import VatInvoiceDocument, UtdDocumentTable, InformationOrganization, Buyer
...
from vat_invoice.utils.vat_invoice_registry_pdf import create_vat_invoice_registry_pdf
...
class VatInvoiceDocumentCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        existing_document = VatInvoiceDocument.objects.filter(name=document_name)
        ...
        if formset.is_valid():
            ...
            form_data = form.cleaned_data
            response = create_vat_invoice_excel(form_data, formset_data, True, True)
            return response
...
def vat_invoice_document(request):
    ...
    documents = VatInvoiceDocument.objects.select_related('organization', 'counterparty').prefetch_related('table_product').filter(user=request.user)
    ...
    return render(request, 'vat_invoice_document_new.html',
...
def vat_invoice_registry(request):
    utd_documents_all = VatInvoiceDocument.objects.select_related('organization', 'counterparty').filter(user=request.user).annotate(
    ...
    html_string = render_to_string('vat_invoice_registry.html', context)

    response = create_vat_invoice_registry_pdf(html_string)
    return response
