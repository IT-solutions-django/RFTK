...
from invoice.models import PkoDocument, InformationOrganization, Buyer
...
from pko.utils.pko_registry_pdf import create_pko_registry_pdf
...
class PkoDocumentCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        existing_document = PkoDocument.objects.filter(name=document_name, date=document_date)
        ...
        form_data = form.cleaned_data
        response = create_pko_excel(form_data, [], True, True)
        return response
...
def pko_document(request):
    ...
    return render(request, 'pko_document_new.html',
...
def pko_registry(request):
    utd_documents_all = PkoDocument.objects.select_related('organization').filter(user=request.user)

    context = {
        'utd_documents_all': utd_documents_all
    }

    html_string = render_to_string('pko_registry.html', context)

    response = create_pko_registry_pdf(html_string)
    return response
