...
def create_ks2_excel(data, formset_data, pdf=False, watch_document=False):
    ...
    if pdf:
        ...
        with open(temp_modified_pdf_path, "rb") as pdf_file:
            ...
            if watch_document:
                response["Content-Disposition"] = "inline; filename=КС-2.pdf"
            else:
                response["Content-Disposition"] = "attachment; filename=КС-2.pdf"
    ...
    response["Content-Disposition"] = f"attachment; filename=КС-2.xlsx"
