...
from invoice.models import ActServiceDocument, CommercialOfferDocumentTable, InformationOrganization, Buyer
...
from act_service.utils.act_services_registry_pdf import create_act_services_registry_pdf
...
def act_service_document(request):
    ...
    documents = ActServiceDocument.objects.select_related('organization', 'counterparty').prefetch_related('table_product').filter(user=request.user)
    if query:
...
def act_services_registry(request):
    utd_documents_all = ActServiceDocument.objects.select_related('organization', 'counterparty').filter(user=request.user).annotate(
    ...
    html_string = render_to_string('act_services_registry.html', context)

    response = create_act_services_registry_pdf(html_string)
    return response
