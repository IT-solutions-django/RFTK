...
from invoice.models import OutlayDocument, CommercialOfferDocumentTable, InformationOrganization, Buyer
...
from outlay.utils.outlay_registry_pdf import create_outlay_registry_pdf
...
def outlay_document(request):
    ...
    documents = OutlayDocument.objects.select_related('organization', 'counterparty').prefetch_related('table_product').filter(user=request.user)
    ...
    return render(request, 'outlay_document_new.html', {'page_obj': page_obj, 'query': query, 'current_page': page_obj.number, 'total_pages': paginator.num_pages, 'organizations': organizations, 'counterparty':  counterparty, 'page_range': page_range, 'date_from': date_from, 'date_to': date_to})


def outlay_registry(request):
    utd_documents_all = OutlayDocument.objects.select_related('organization', 'counterparty').filter(user=request.user).annotate(
    ...
    html_string = render_to_string('outlay_registry.html', context)

    response = create_outlay_registry_pdf(html_string)
    return response
